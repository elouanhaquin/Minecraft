...
void main()
{
  ...
  float x = 0 + playerPos.x;                          //time is between -0.2;1.8
  float z =  100 + playerPos.z;

  x = isGoingUp < 0 ? -500 + playerPos.x : 450 + playerPos.x;

  if(y == 399){
    x = min((-time2) * 1.2f  + 575+ playerPos.x , 450 + playerPos.x);
    x = max(x, -500 + playerPos.x);
  }
  ...
  if((( FragPos.y < y + 50 && FragPos.y > y - 50 )
    && (FragPos.z < z + 50 && FragPos.z > z - 50)))
   {
      ...
      else{float blue = (FragPos.y * 0.0002f);  FragColor = vec4(0.53f,0.85f,0.98f,1) *(1-blue) * time ;}
   }
  else{*/
